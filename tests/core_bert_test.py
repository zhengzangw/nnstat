...
def test_math_basic_tensor():
    ...
    assert torch.allclose((state_dict + 1).at(0).sum(), state_dict.at(0).sum() + state_dict.at(0).numel())
...
def test_filter():
    assert state_dict.filter(pattern=".*query.*").numel() == len(state_dict.flatten(pattern=".*query.*"))


def test_create():
    sd1 = nnstat.from_state_dict(model.state_dict())
    sd2 = nnstat.from_weight(model)
    assert torch.equal(sd1.at(-1), sd2.at(-1))


def test_describe_tensor():
    r1 = state_dict.describe(lw=True, pattern=".*query.*", display=False, exlude_keys=["no"])
    r2 = nnstat.from_weight(model, pattern=".*query.*").describe(lw=True, display=False, exlude_keys=["no"])
